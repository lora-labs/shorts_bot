...
Flow
----
1. User sends /start and then describes the video idea.
2. :func:`generate_video` hands the idea to :class:`ScenePipeline`
   (Qwen -> SDXL -> LTX-2.3) and waits for the finished ``final.mp4``.
3. The bot tries to send the file directly; if it is too big for the
   Telegram Bot API (or Telegram rejects it) it falls back to uploading
   the file to Google Drive and returning a public link.
...
async def generate_video(prompt_text: str, request_id: str | int = 0) -> Path:
    """Run the end-to-end ScenePipeline and return the final mp4 path.

    The blocking pipeline runs in a thread so the event loop stays responsive.
    """
    run_dir = OUTPUT_DIR / f"run_{request_id}"
    config = PipelineConfig(comfyui_url=COMFYUI_URL, output_dir=run_dir)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text(
        "Привет! Опиши идею видео для TikTok — я сгенерирую сценарий, "
        "отрендерю ключевые кадры, оживлю их в LTX-2.3 и пришлю готовое видео."
    )
    return ASK_VIDEO_TYPE


async def ask_video_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user_text = update.message.text or ""
    context.user_data["prompt"] = user_text
    await update.message.reply_text(
        "Принято. Генерирую сценарий, кадры и видео — это может занять несколько минут."
    )

    try:
        video_path = await generate_video(user_text, request_id=update.effective_chat.id)
...
def main() -> None:
    ...
    log.info("Бот запущен, COMFYUI_URL=%s output=%s", COMFYUI_URL, OUTPUT_DIR)
    app.run_polling()
